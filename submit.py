...
class submit:
    ...
    def __init__(self):
        self.server_key = ""  # server酱key,登录之后就可使用
    ...
    def login(self,name,password):
        ...
        r = self.s.post("https://itsapp.bjut.edu.cn/uc/wap/login/check", data=payload)
    ...
    def get_yesterday(self):
        data = self.s.get("https://itsapp.bjut.edu.cn/ncov/api/default/daily?xgh=0&app_id=bjut")
        json_data = data.json()
    ...
    def submit(self, old: dict):
        ...
        r = self.s.post("https://itsapp.bjut.edu.cn/ncov/wap/default/save", data=new_daily)
        ...
        if result.get('m') == "操作成功":
            print("打卡成功")
            if self.server_key != "":
                self.send_message(self.server_key, result.get('m'), new_daily)
            return '打卡成功'
        else:
            print("打卡失败，错误信息: ", r.json().get("m"))
            if self.server_key != "":
                self.send_message(self.server_key, result.get('m'), new_daily)
            return '打卡失败，错误信息：'+r.json().get("m")


    # 微信通知
    def send_message(self,key, message, clock_info):
